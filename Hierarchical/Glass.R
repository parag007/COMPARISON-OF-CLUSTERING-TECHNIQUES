...
pca <- princomp(glass[, 1:9], cor=T) # principal components analysis using correlation matrix
pc.comp <- pca$scores
...
aggregate(glass[, 2:10],by=list(km$cluster),mean)
...
#set.seed(12)
d_glass<- dist(clustering.data)
hclusters <- hclust(d_glass, method = "average")
clusterCut <- cutree(hclusters, 7)
clusterCut
table(clusterCut, glass$Type)
aggregate(glass[,2:10],by=list(clusterCut),mean)
...
#Single link
d_glass<- dist(clustering.data)
hclusters <- hclust(d_glass, method = "single")
clusterCut <- cutree(hclusters, 7)
clusterCut
table(clusterCut, glass$Type)
aggregate(glass[,2:10],by=list(clusterCut),mean)
...
#Complete link
d_glass<- dist(clustering.data)
hclusters <- hclust(d_glass, method = "complete")
clusterCut <- cutree(hclusters, 7)
clusterCut
table(clusterCut, glass$Type)
aggregate(glass[,2:10],by=list(clusterCut),mean)
